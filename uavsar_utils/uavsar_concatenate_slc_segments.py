...
def createParser():
    EXAMPLE = """example:
      uavsar_concatenate_slc_segments.py -w ./ -s1 SLC_seg1/20091118/20091118.slc -s 1 2 3 -o SLC_merged
    """
    ...
    parser.add_argument('-w', '--work_dir', dest='work_dir', type=str, required=True,
            help='work dir containing all SLC segments.')
    parser.add_argument('-s1', '--segment1_slc', dest='seg1_slc', type=str, required=True,
            help='path to first segment SLC.')
    ...
    parser.add_argument('-o', '--slc_out', dest='slc_merged_dir', type=str, required=True,
            help='concatenated SLC directory.')
    return parser
...
def main(iargs=None):
    ...
    #print(inps)
    work_dir = os.path.abspath(inps.work_dir) # go to work dir
    os.chdir(work_dir)
    print('Go to directory: '+ work_dir);
    slc_merged_dir = os.path.abspath(inps.slc_merged_dir);#print(slc_merged_dir)
    segments = inps.segments;
    #print('segments:',segments);
    if not os.path.exists(inps.slc_merged_dir):
        os.mkdir(inps.slc_merged_dir)
    seg1_slc_path = os.path.abspath(inps.seg1_slc);
    seg1_dir = seg1_slc_path.split('/')[-3];#print(seg1_dir)
    a = os.path.basename(seg1_slc_path).split('.')[0];
    slc_segs = [];
